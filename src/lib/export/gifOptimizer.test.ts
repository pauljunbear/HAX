...
describe('GifOptimizer', () => {
  ...
  describe('generateReport', () => {
    ...
    it('should provide relevant recommendations', () => {
      const frames = Array(60).fill(null).map(() => createTestCanvas(800, 600, 'complex'));
      ...
      const hasFrameRecommendation = report.recommendations.some(r => 
        r.includes('frame') || r.includes('reducing')
      );
    ...
    it('should recommend color optimization for high color count', () => {
      ...
      if (report.analysis.colorCount > 200) {
        const hasColorRecommendation = report.recommendations.some(r => 
          r.includes('color')
        );
  ...
  describe('Quality Presets', () => {
    it('should have valid preset configurations', () => {
      ...
      expect(GIF_QUALITY_PRESETS.minimal.quality).toBeLessThan(GIF_QUALITY_PRESETS.highQuality.quality);
      
      // Check high quality preset has largest max size
      expect(GIF_QUALITY_PRESETS.highQuality.maxSizeKB).toBeGreaterThan(GIF_QUALITY_PRESETS.minimal.maxSizeKB!);
    });
    ...
    it('should apply presets correctly in optimization', () => {
      ...
      // Minimal should result in smaller or equal estimated size
      expect(minimalResult.estimatedSizeKB).toBeLessThanOrEqual(webResult.estimatedSizeKB * 1.5);
    });
  });
  ...
  describe('Performance', () => {
    it('should analyze frames efficiently', () => {
      const frames = Array(10).fill(null).map(() => createTestCanvas(800, 600, 'complex'));
    ...
    it('should handle large frame counts without excessive memory usage', () => {
      const frames = Array(50).fill(null).map(() => createTestCanvas(400, 300, 'simple'));
