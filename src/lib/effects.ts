...
export const effectsConfig: Record<string, EffectConfig> = {
  ...
  delaunay: {
    ...
    settings: [
      ...
      {
        id: 'fillTriangles',
        label: 'Fill Triangles (0=No, 1=Yes)',
        min: 0,
        max: 1,
        defaultValue: 1,
        step: 1,
...
const createDelaunayEffect = (settings: Record<string, number>) => {
  ...
  function computeDelaunayTriangulation(
    ...
    for (const point of points) {
      ...
      for (const triangle of badTriangles) {
        for (let i = 0; i < 3; i++) {
          const edge: [(typeof points)[0], (typeof points)[0]] = [
            triangle[i],
            triangle[(i + 1) % 3],
          ];
    ...
    return triangles.filter(triangle => !superTriangle.some(sp => triangle.includes(sp as any)));
  }
  ...
  function isInCircumcircle(
    ...
    const det =
      ax * (by * cx * cx + by * cy * cy - bx * bx * cy - bx * cy * cy) -
