  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      if (projection > 0 && perpendicular <= maxSpread) {
        ...
        const perpFalloff =
          distanceFromRay <= maxSpread * softness
            ? 1
...
// Apply blend modes
function applyBlendMode(base: number, overlay: number, mode: string): number {
  const b = base / 255;
  const o = overlay / 255;
  let result = 0;
...
export const LightLeakFilter = function (this: Konva.Image, imageData: ImageData) {
  ...
  const directionalGradient = createDirectionalGradient(
    width,
    height,
    positionX,
    positionY,
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      const combinedIntensity =
        Math.max(radialIntensity, directionalIntensity * 0.7) * noiseValue * intensity * opacity;
