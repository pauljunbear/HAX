...
export const ensureKonvaInitialized = () => {
  ...
  try {
    ...
    const konvaWithFilters = Konva as { Filters?: Record<string, unknown> };
    if (!konvaWithFilters.Filters) {
      konvaWithFilters.Filters = {};
    }
  } catch {
    // ignore
  }
...
export const validateSettings = (effectId: string, settings: EffectSettings): EffectSettings => {
  ...
  Object.entries(settings).forEach(([key, value]) => {
    ...
    if (settingConfig) {
      ...
      validatedSettings[key] = Math.min(Math.max(value, settingConfig.min), settingConfig.max);
    }
  });
