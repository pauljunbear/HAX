      ...
      {activeEffect && currentEffectSettings.length > 0 && (
        <div className="py-4 px-5 border-b border-dark-border">
          <h3 className="text-xs font-medium text-dark-text mb-4 flex items-center">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 mr-1.5 text-primary-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {currentEffectSettings.map(setting => {
            if (setting.type === 'color') {
              return (
                <div key={setting.id} className="mb-4 p-2 bg-yellow-900/50 border border-yellow-700/50 rounded-md">
                  <p className="text-xs text-yellow-300">Color settings for '{setting.label}' require a color picker UI (not implemented yet).</p>
                </div>
              );
            }
