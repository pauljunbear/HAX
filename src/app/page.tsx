...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-dark-bg font-sans text-dark-text">
      {/* Header - Responsive */}
      <header className="h-14 md:h-14 border-b border-dark-border bg-dark-bg sticky top-0 z-50 flex items-center justify-between px-4 md:px-6">
        <div className="flex items-center">
          {/* Logo and title */}
          <h1 className="text-base md:text-lg font-semibold text-dark-text">
            <span className="text-primary-accent">Imager</span>
          </h1>
          <div className="hidden sm:block ml-3 text-[10px] font-medium bg-dark-border text-dark-textMuted px-2 py-0.5 rounded-full">Real-time Effects</div>
        </div>
          ...
          {selectedImage && (
            <>
              <button
                ...
                className="px-2.5 md:px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-medium rounded-lg transition-all duration-200 flex items-center"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 md:mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <button
                ...
                className="px-2.5 md:px-3 py-1.5 bg-primary-accent hover:bg-primary-accent/90 text-white text-xs font-medium rounded-lg transition-all duration-200 flex items-center shadow-sm"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 md:mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="flex items-center gap-3">
          ...
          {selectedImage && (
            <button
              ...
              className="mobile-menu-button md:hidden p-2 rounded-lg hover:bg-dark-surface transition-colors"
            >
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 md:mr-1" fill="currentColor" viewBox="0 0 24 24">
      ...
      <main className="flex h-[calc(100vh-3.5rem)] relative">
        {/* Desktop sidebar */}
        <div className="hidden md:flex w-80 border-r border-dark-border bg-dark-surface overflow-y-auto flex-col">
          {/* New Control Panel */}
          <ControlPanelV3
        ...
        <div className="flex-1 relative overflow-hidden bg-dark-bg">
          {imageLoading ? (
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center text-dark-textMuted p-6">
              <div className="w-20 h-20 md:w-28 md:h-28 mb-6 rounded-2xl bg-dark-surface flex items-center justify-center">
              ...
              <h2 className="text-base md:text-lg font-medium text-dark-text mb-2">Drop Image or Click to Upload</h2>
              <p className="text-xs text-dark-textMuted mb-6 max-w-md text-center">
                Supported formats: JPG, PNG, GIF, WebP
              </p>
              <label className="bg-primary-accent hover:bg-opacity-90 cursor-pointer px-6 py-3 md:px-4 md:py-2 text-white text-sm md:text-xs font-medium rounded-md transition-colors">
                <input
                  type="file"
                  accept="image/*"
                  className="hidden"
        ...
        {/* Mobile bottom sheet */}
        {selectedImage && (
          <div className={`mobile-menu md:hidden fixed inset-x-0 bottom-0 bg-dark-surface border-t border-dark-border transform transition-transform duration-300 ease-in-out z-40 ${
            mobileMenuOpen ? 'translate-y-0' : 'translate-y-full'
          }`} style={{ maxHeight: '70vh' }}>
            {/* Handle bar */}
            <div className="flex justify-center py-2">
              <div className="w-12 h-1 bg-dark-border rounded-full"></div>
            </div>
